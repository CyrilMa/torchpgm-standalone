...
class DReLULayer(Layer):
    ...
    def update_params(self):
        self.params = nn.ParameterList([nn.Parameter(self.params[0].clamp(1e-3, 1e5)),
                                        nn.Parameter(self.params[1].clamp(1-1e-3, 1e-3)),
                                        nn.Parameter(self.params[2].clamp(-1e2, 1e2)),
                                        nn.Parameter(self.params[3].clamp(-1e2, 1e2))])
    ...
    def mean(self, probas):
        ...
        mu = p_plus*mu_plus + p_minus*mu_minus
